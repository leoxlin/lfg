...
- Outside a git repo, `lfg` asks you to pick one from `$LFG_SOURCE_DIR` with `fzf`; the selector uses a rounded border labeled `Select a repo` around the `repo> ` prompt. Repos are ranked by most recent selection, then by name.
...
- When not already inside a linked worktree, `lfg` asks you to pick an existing worktree branch or type a new branch name to create one; the selector uses a rounded border labeled `Select or create worktree branch` around the `worktree> ` prompt. Branches are ranked by most recent selection, then by name.
...
- `lfg` records agent entrypoint, repo, and branch selections in `store/recency` under the `lfg` install directory (`~/.local/share/lfg/store/recency` by default; override with `LFG_STORE_DIR`) to rank future picks by recency. Branch picks are ranked per repo. The store survives reinstalls and `lfg --update`; delete the file to reset the ranking.
- When `LFG_SMART_MODE` is set and `lfg` runs without an entrypoint argument, `lfg` discovers the available agent entrypoints from the entrypoint completion suggestions (keeping only commands found on `PATH`) and asks you to pick one from an `fzf` selector using a rounded border labeled `Select an agent` around the `agent> ` prompt. Entrypoints are ranked by most recent selection, then by name. Passing an entrypoint explicitly (`lfg codex`) skips the picker. If no completion entrypoints are available on `PATH`, `lfg` prints an error instead of launching anything.
...
| `LFG_SMART_MODE` | unset | When set, `lfg` without an entrypoint argument picks one from an `fzf` selector of available agent entrypoints instead of using `LFG_DEFAULT_AGENT_COMMAND`. |
| `LFG_STORE_DIR` | `store` under the `lfg` install directory | Directory holding the recency store that ranks entrypoint, repo, and branch picks. |
